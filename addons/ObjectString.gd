...
@export var obj_rotation: float = 0.0:
	...
	get:
		return obj_rotation
@export var rotation_mirrored: bool = true:
	set(value):
		rotation_mirrored = value
		spawn_objects()
	get:
		return rotation_mirrored
...
func spawn_objects():
	...
	for i in range(0, count):
		...
		if right:
			var inst = item.instantiate()
			var rotation = -deg_to_rad(obj_rotation) if rotation_mirrored else PI-deg_to_rad(obj_rotation)
			inst.set_global_transform(transform.translated_local(Vector3.RIGHT * distance_from_center).rotated_local(Vector3.UP, rotation))
			add_child(inst)
			spawned.append(inst)
